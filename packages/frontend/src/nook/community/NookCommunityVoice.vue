...
<section class="_panel" :class="$style.root">
	...
	<template v-else>
		...
		<div v-if="!canSpeak" :class="$style.status">🔇 Listen only</div>
		...
		<details v-if="canManage" :class="$style.admin">
			<summary>{{ l.settings }}</summary>
...
import { canReceiveNookVoiceAudio, parseNookVoiceSignalPayload } from './voice-client.js';
...
let localStream:MediaStream | null = null; let acquiringLocalAudio = false; let iceServers:RTCIceServer[] = []; let iceTransportPolicy:RTCIceTransportPolicy = 'all'; let approvedMusicUrl:string | null = null; let speakingPeers = new Set<string>(); const pcs = new Map<string, RTCPeerConnection>(); let signalTimer:number | undefined; let heartbeatTimer:number | undefined; let ttsTimer:number | undefined; const ttsSeen = new Set<string>(); let ttsInitialized = false;
...
function applySpeakingPeers(ids:string[]) {speakingPeers = new Set(ids); for (const [userId, pc] of pcs) {const enabled = canReceiveNookVoiceAudio(ids, userId); for (const receiver of pc.getReceivers()) if (receiver.track?.kind === 'audio')receiver.track.enabled = enabled;}}

async function ensurePeer(userId:string, offer:boolean) {if (pcs.has(userId)) return pcs.get(userId)!; const pc = new RTCPeerConnection({ iceServers, iceTransportPolicy }); pcs.set(userId, pc); if (localStream) for (const track of localStream.getTracks())pc.addTrack(track, localStream); pc.onicecandidate = e => {if (e.candidate) void signal(userId, 'ice', e.candidate.toJSON());}; pc.ontrack = e => {if (e.track.kind === 'audio')e.track.enabled = canReceiveNookVoiceAudio([...speakingPeers], userId); const stream = e.streams[0] ?? new MediaStream([e.track]); remoteStreams.value = { ...remoteStreams.value, [userId]: stream };}; pc.onconnectionstatechange = () => {if (['failed', 'closed'].includes(pc.connectionState))closePeer(userId);}; if (offer) {const description = await pc.createOffer(); await pc.setLocalDescription(description); await signal(userId, 'offer', description);} return pc;}

async function ensureLocalAudio() {if (!joined.value || !canSpeak.value || localStream || acquiringLocalAudio) return; acquiringLocalAudio = true; try {const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false }); if (!joined.value || !canSpeak.value) {stream.getTracks().forEach(track => track.stop()); return;} const track = stream.getAudioTracks()[0]; if (!track) {stream.getTracks().forEach(item => item.stop()); return;}localStream = stream; for (const [userId, pc] of pcs) {try {const sender = pc.getSenders().find(item => item.track?.kind === 'audio'); if (sender) {await sender.replaceTrack(track); continue;}pc.addTrack(track, stream); if (pc.signalingState === 'stable') {const description = await pc.createOffer(); await pc.setLocalDescription(description); await signal(userId, 'offer', description);}} catch {closePeer(userId);}}} catch {localStream = null;} finally {acquiringLocalAudio = false;}}
...
async function pollSignals() {if (!joined.value) return; let signals:Array<{ fromUserId: string; type: 'offer' | 'answer' | 'ice'; payload: string }>; try {signals = await nookApi('nook/community/voice/signals', { channelId: props.channel.id, sessionId: sessionId.value });} catch {return;} for (const item of signals) {try {const payload = parseNookVoiceSignalPayload(item.payload); if (payload == null) {closePeer(item.fromUserId); continue;} const pc = await ensurePeer(item.fromUserId, false); if (item.type === 'offer') {await pc.setRemoteDescription(payload as RTCSessionDescriptionInit); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); await signal(item.fromUserId, 'answer', answer);} else if (item.type === 'answer') {if (pc.signalingState !== 'stable') await pc.setRemoteDescription(payload as RTCSessionDescriptionInit);} else {await pc.addIceCandidate(payload as RTCIceCandidateInit);}} catch {closePeer(item.fromUserId);}}}

async function doHeartbeat() {if (!joined.value) return; try {const state = await nookApi<VoiceHeartbeat>('nook/community/voice/heartbeat', { channelId: props.channel.id, sessionId: sessionId.value }); heartbeat.value = state; peers.value = state.peers; const couldSpeak = canSpeak.value; canSpeak.value = state.canSpeak; applySpeakingPeers(state.speakingPeerIds); if (!state.canSpeak && localStream) {localStream.getTracks().forEach(track => track.stop()); localStream = null;} else if (state.canSpeak && !couldSpeak) {await ensureLocalAudio();} for (const peer of state.peers) await ensurePeer(peer, ($i?.id ?? '') < peer); applySpeakingPeers(state.speakingPeerIds); for (const peer of [...pcs.keys()]) if (!state.peers.includes(peer))closePeer(peer); applyMusic(state); void pollTts(state);} catch {await leave();}}

async function join() {const result = await nookApi<{ sessionId: string; peers: string[]; speakingPeerIds: string[]; canSpeak: boolean; iceServersJson: string; iceTransportPolicy: 'all' | 'relay' }>('nook/community/voice/join', { communityId: props.communityId, channelId: props.channel.id }); sessionId.value = result.sessionId; canSpeak.value = result.canSpeak; iceTransportPolicy = result.iceTransportPolicy; speakingPeers = new Set(result.speakingPeerIds); try {iceServers = JSON.parse(result.iceServersJson);} catch {iceServers = [];}joined.value = true; await ensureLocalAudio(); peers.value = result.peers; for (const peer of result.peers) await ensurePeer(peer, ($i?.id ?? '') < peer); applySpeakingPeers(result.speakingPeerIds); signalTimer = window.setInterval(() => void pollSignals(), 1000); heartbeatTimer = window.setInterval(() => void doHeartbeat(), 10000); ttsTimer = window.setInterval(() => {if (heartbeat.value) void pollTts(heartbeat.value);}, 2500); await doHeartbeat();}

async function leave() {if (!joined.value) return; joined.value = false; if (signalTimer)window.clearInterval(signalTimer); if (heartbeatTimer)window.clearInterval(heartbeatTimer); if (ttsTimer)window.clearInterval(ttsTimer); for (const pc of pcs.values())pc.close(); pcs.clear(); speakingPeers.clear(); remoteStreams.value = {}; localStream?.getTracks().forEach(t => t.stop()); localStream = null; speechSynthesis.cancel(); const el = musicAudio.value; if (el) {el.pause(); el.removeAttribute('src'); el.load();}approvedMusicUrl = null; try {await nookApi('nook/community/voice/leave', { channelId: props.channel.id, sessionId: sessionId.value });} catch {}sessionId.value = '';}
...
function resumeMusicLocally() {const el = musicAudio.value; const music = heartbeat.value?.music; if (!el || !heartbeat.value?.config.musicEnabled || music?.url == null) return; if (approvedMusicUrl !== music.url) {el.src = music.url; approvedMusicUrl = music.url;} const elapsed = music.playing ? (Date.now() - new Date(music.updatedAt).getTime()) / 1000 : 0; const desired = Math.max(0, music.positionSeconds + elapsed); try {el.currentTime = desired;} catch {} void el.play().catch(() => {});}
