...
function applySpeakingPeers(ids:string[]){speakingPeers=new Set(ids);for(const [userId,pc] of pcs){const enabled=speakingPeers.has(userId);for(const receiver of pc.getReceivers())if(receiver.track?.kind==='audio')receiver.track.enabled=enabled;}}
async function ensurePeer(userId:string,offer:boolean){if(pcs.has(userId))return pcs.get(userId)!;const pc=new RTCPeerConnection({iceServers,iceTransportPolicy});pcs.set(userId,pc);if(localStream)for(const track of localStream.getTracks())pc.addTrack(track,localStream);pc.onicecandidate=e=>{if(e.candidate)void signal(userId,'ice',e.candidate.toJSON());};pc.ontrack=e=>{if(e.track.kind==='audio')e.track.enabled=speakingPeers.has(userId);const stream=e.streams[0]??new MediaStream([e.track]);remoteStreams.value={...remoteStreams.value,[userId]:stream};};pc.onconnectionstatechange=()=>{if(['failed','closed'].includes(pc.connectionState))closePeer(userId);};if(offer){const description=await pc.createOffer();await pc.setLocalDescription(description);await signal(userId,'offer',description);}return pc;}
