...
let localStream:MediaStream|null=null; let iceServers:RTCIceServer[]=[]; let iceTransportPolicy:RTCIceTransportPolicy='all'; let approvedMusicUrl:string|null=null; const pcs=new Map<string,RTCPeerConnection>(); let signalTimer:number|undefined; let heartbeatTimer:number|undefined; let ttsTimer:number|undefined; const ttsSeen=new Set<string>(); let ttsInitialized=false;
...
async function ensurePeer(userId:string,offer:boolean){if(pcs.has(userId))return pcs.get(userId)!;const pc=new RTCPeerConnection({iceServers,iceTransportPolicy});pcs.set(userId,pc);if(localStream)for(const track of localStream.getTracks())pc.addTrack(track,localStream);pc.onicecandidate=e=>{if(e.candidate)void signal(userId,'ice',e.candidate.toJSON());};pc.ontrack=e=>{const stream=e.streams[0]??new MediaStream([e.track]);remoteStreams.value={...remoteStreams.value,[userId]:stream};};pc.onconnectionstatechange=()=>{if(['failed','closed'].includes(pc.connectionState))closePeer(userId);};if(offer){const description=await pc.createOffer();await pc.setLocalDescription(description);await signal(userId,'offer',description);}return pc;}
...
async function doHeartbeat(){if(!joined.value)return;try{const state=await nookApi<VoiceHeartbeat>('nook/community/voice/heartbeat',{channelId:props.channel.id,sessionId:sessionId.value});heartbeat.value=state;peers.value=state.peers;canSpeak.value=state.canSpeak;if(!state.canSpeak&&localStream){localStream.getTracks().forEach(track=>track.stop());localStream=null;}for(const peer of state.peers)await ensurePeer(peer,($i?.id??'')<peer);for(const peer of [...pcs.keys()])if(!state.peers.includes(peer))closePeer(peer);applyMusic(state);void pollTts(state);}catch{await leave();}}
async function join(){const result=await nookApi<{sessionId:string;peers:string[];canSpeak:boolean;iceServersJson:string;iceTransportPolicy:'all'|'relay'}>('nook/community/voice/join',{communityId:props.communityId,channelId:props.channel.id});sessionId.value=result.sessionId;canSpeak.value=result.canSpeak;iceTransportPolicy=result.iceTransportPolicy;try{iceServers=JSON.parse(result.iceServersJson);}catch{iceServers=[];}if(canSpeak.value){try{localStream=await navigator.mediaDevices.getUserMedia({audio:true,video:false});}catch{localStream=null;}}joined.value=true;peers.value=result.peers;for(const peer of result.peers)await ensurePeer(peer,($i?.id??'')<peer);signalTimer=window.setInterval(()=>void pollSignals(),1000);heartbeatTimer=window.setInterval(()=>void doHeartbeat(),10000);ttsTimer=window.setInterval(()=>{if(heartbeat.value)void pollTts(heartbeat.value);},2500);await doHeartbeat();}
