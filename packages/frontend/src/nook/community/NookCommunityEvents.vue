...
<section class="_gaps">
	...
	<article v-for="event in events" :key="event.id" class="_panel" :class="$style.event">
		<h3>{{ event.cancelledAt ? '🚫 ' : '📅 ' }}{{ event.title }}</h3><p v-if="event.description">{{ event.description }}</p><NookAutoTranslation v-if="event.description" kind="communityEvent" :objectId="event.id" :text="event.description"/>
