...
<div class="_gaps">
	...
	<template v-else-if="detail">
		<div class="_panel" :class="$style.top">
			<div>
				<strong>{{ l.community }}</strong>
				<small> · {{ detail.memberCount }} {{ l.members }}</small>
				<span :class="$style.ageMode">{{ ageModeLabel }}</span>
			</div>
			<div :class="$style.translation"><label><input v-model="autoTranslate" type="checkbox"> 🌐 {{ l.autoTranslate }}</label><input v-model="targetLang" maxlength="24" :title="l.translationLanguage"></div>
		</div>
...
const ageModeLabel = computed(() => {
	if (detail.value?.ageMode === 'minors_only') return l.ageModeMinorsOnly;
	if (detail.value?.ageMode === 'adults_only') return l.ageModeAdultsOnly;
	return l.ageModeMixed;
});
...
async function load() {
	...
	try {
		...
		pins.value = isActiveMember.value ? await nookApi('nook/community/pins/list', { communityId: props.communityId, channelId: null }) : [];
		if (!tabs.value.some(item => item.key === tab.value)) tab.value = 'home';
	} finally {
		loading.value = false;
	}
}
...
async function join() {
	try {
		if (inviteToken.value) {
			...
			const result = await nookApi<{ status: string }>('nook/community/join', { communityId: props.communityId, message: joinMessage.value || null });
