...
<div class="_gaps">
	...
	<template v-else-if="detail">
		<div class="_panel" :class="$style.top"><div><strong>{{ l.community }}</strong><small> · {{ detail.memberCount }} {{ l.members }}</small><span :class="$style.ageMode">{{ ageModeLabel }}</span></div><div :class="$style.translation"><label><input v-model="autoTranslate" type="checkbox"> 🌐 {{ l.autoTranslate }}</label><input v-model="targetLang" maxlength="24" :title="l.translationLanguage"></div></div>
