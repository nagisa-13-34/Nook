...
<div class="_gaps">
	...
	<template v-else-if="detail">
		...
		<section v-if="!detail.membership" class="_panel" :class="$style.box"><h3>{{ l.rules }}</h3><ol><li v-for="rule in rules" :key="rule.id"><strong>{{ rule.title }}</strong> — {{ rule.body }}</li></ol><textarea v-if="detail.joinMode==='approval'" v-model="joinMessage" maxlength="1024" placeholder="Message"></textarea><input v-if="detail.joinMode==='invite'||detail.joinMode==='private'" v-model="inviteToken" placeholder="Invite token"><button class="_button" :class="$style.primary" @click="join">{{ detail.joinMode==='approval'?l.apply:l.join }}</button><div v-if="joinStatus">{{ joinStatus }}</div></section>
		<template v-else>
			...
			<NookCommunityMembers v-else-if="tab==='members'" :community-id="communityId" :can-manage="can('members.manage')"/>
			<NookCommunityBots v-else-if="tab==='bots'" :community-id="communityId" :can-manage="can('bots.manage')"/>
...
function can(permission:string){const values=detail.value?.membership?.permissions??[];return values.includes('*')||values.includes(permission);}
const tabs=computed(()=>[{key:'home',label:'Home',icon:'🏠'},{key:'channels',label:l.channels,icon:'#'},{key:'announcements',label:l.announcements,icon:'📢'},{key:'events',label:l.events,icon:'📅'},{key:'members',label:l.members,icon:'👥'},{key:'bots',label:l.bots,icon:'🤖'},...(can('community.manage')||can('roles.manage')||can('rules.manage')||can('channels.manage')?[{key:'admin',label:l.admin,icon:'⚙️'}]:[])]);
async function load(){loading.value=true;try{detail.value=await nookApi('nook/community/show',{communityId:props.communityId});rules.value=await nookApi('nook/community/rules/list',{communityId:props.communityId});pins.value=detail.value.membership?await nookApi('nook/community/pins/list',{communityId:props.communityId,channelId:null}):[];}finally{loading.value=false;}}
