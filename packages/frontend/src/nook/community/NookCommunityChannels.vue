...
<section :class="$style.layout">
	...
	<div :class="$style.main">
		<NookCommunityVoice v-if="voiceEnabled&&selected?.kind==='voice'" :communityId="communityId" :channel="selected" :channels="channels" :canManage="canManageVoice"/>
		<div v-else-if="selected" class="_gaps">
			...
			<div class="_panel" :class="$style.messages"><article v-for="message in messages" :key="message.id" :class="$style.message"><small>{{ message.botId ? '🤖 '+message.botId : message.userId }} · {{ new Date(message.createdAt).toLocaleTimeString() }}</small><div :class="$style.body">{{ message.body }}</div><NookAutoTranslation kind="communityMessage" :objectId="message.id" :text="message.body"/></article></div>
...
onMounted(async() => {await loadChannels(); timer = window.setInterval(() => void loadMessages(), 3000);}); onBeforeUnmount(() => {if (timer)window.clearInterval(timer);});
</script>
