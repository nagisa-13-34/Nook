...
async function unfavorite() {
	if (!channel.value) return;
	const confirm = await os.confirm({ type: 'warning', text: i18n.ts.unfavoriteConfirm });
...
async function mute() {
	...
	const expiresAt = period === 'indefinitely' ? null
		...
		: null;
	os.apiWithDialog('channels/mute/create', { channelId: _channel.id, expiresAt }).then(() => {
...
const headerActions = computed(() => {
	...
	if (isSupportShare()) {
		headerItems.push({
			...
			handler: async (): Promise<void> => {
				if (!channel.value) return;
				navigator.share({ title: channel.value.name, text: channel.value.description ?? undefined, url: `${url}/channels/${channel.value.id}` });
			},
		});
	}
	if (!channel.value.isMuting) {
		headerItems.push({ icon: 'ti ti-volume', text: i18n.ts.mute, handler: async () => { await mute(); } });
	} else {
...
.banner { position: relative; height: 200px; background-position: center; background-size: cover; }
.bannerFade { position: absolute; bottom: 0; left: 0; width: 100%; height: 64px; background: linear-gradient(0deg, var(--MI_THEME-panel), color(from var(--MI_THEME-panel) srgb r g b / 0)); }
.bannerStatus { position: absolute; z-index: 1; bottom: 16px; right: 16px; padding: 8px 12px; font-size: 80%; background: rgba(0, 0, 0, 0.7); border-radius: 6px; color: #fff; }
.description { padding: 16px; }
.sensitiveIndicator { position: absolute; z-index: 1; bottom: 16px; left: 16px; background: rgba(0, 0, 0, 0.7); color: var(--MI_THEME-warn); border-radius: 6px; font-weight: bold; font-size: 1em; padding: 4px 7px; }
</style>