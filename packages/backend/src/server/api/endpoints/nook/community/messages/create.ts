...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		try {
			...
			await assertNookCommunityChannelAdultBoundary(
				this.db,
			...
			if (error instanceof NookCommunityAccessError || error instanceof NookCommunityChannelError || error instanceof NookCommunityCommunicationError) {
		...
		try {
			const message = await createNookCommunityMessage(this.db, this.idService, {
				communityId: ps.communityId,
				channelId: ps.channelId,
				userId: me.id,
				body: ps.body,
				replyToId: ps.replyToId ?? null,
			});
