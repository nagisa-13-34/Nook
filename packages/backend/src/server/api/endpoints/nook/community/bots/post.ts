...
export const paramDef = {
	type: 'object',
	properties: {
		botId: { type: 'string', format: 'misskey:id' },
		secret: { type: 'string', minLength: 32, maxLength: 256 },
		channelId: { type: 'string', format: 'misskey:id' },
		body: { type: 'string', minLength: 1, maxLength: 8000 },
		replyToId: { type: 'string', format: 'misskey:id', nullable: true },
	},
	required: ['botId', 'secret', 'channelId', 'body'],
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async ps => {
			...
			const channels = await this.db.query<Array<{ id: string; kind: string; archivedAt: Date | null }>>(
				'SELECT "id","kind","archivedAt" FROM "nook_community_channel" WHERE "communityId"=$1 AND "id"=$2 LIMIT 1',
				[bot.communityId, ps.channelId],
			);
			if (channels[0] == null || channels[0].archivedAt != null || channels[0].kind === 'voice') throw new ApiError(meta.errors.forbidden);
