...
export const meta = { tags: ['channels'], requireCredential: true, kind: 'read:channels',
	...
	errors: { forbidden: { message: 'You must be a community member.', code: 'FORBIDDEN', id: '03b59256-3ea0-483a-a35b-b6fcb99e7e1a' } },
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(@Inject(DI.db) private db: DataSource) { super(meta, paramDef, async (ps, me) => { try { await requireNookCommunityMember(this.db, ps.communityId, me.id); } catch (error) { if (error instanceof NookCommunityAccessError) throw new ApiError(meta.errors.forbidden); throw error; } return await listNookCommunityRoles(this.db, ps.communityId); }); }
}
