...
function hasAgeBoundary(senderAgeGroup: NookAgeGroup, recipient: CommunityUserRow | null): { sender: boolean; recipient: boolean } {
	...
	const senderNeedsPermission = !isNookAdultAgeGroup(senderAgeGroup) && (
		...
		isNookAdultAgeGroup(recipientAgeGroup)
	);
...
export async function assertNookCommunityAdultBoundaryForUserIds(
	...
	for (const targetUserId of uniqueTargetIds) {
		...
		if (boundary.sender && !actorDecisionChecked) {
			...
			if (!engine.evaluate(getSubject(actor), permission).allowed) {
				throw new NookCommunityCommunicationError('ADULT_BOUNDARY');
			}
		}

		if (boundary.recipient && target != null) {
			if (!engine.evaluate(getSubject(target), permission).allowed) {
				throw new NookCommunityCommunicationError('ADULT_BOUNDARY');
			}
		}
	}
...
export async function listNookCommunityChannelAudienceUserIds(
	...
	const channelRows = await db.query<Array<{ ownerId: string | null; allowedRoleIds: unknown }>>(
		`SELECT c."userId" AS "ownerId", cc."allowedRoleIds"
		 ...
		 WHERE cc."communityId" = $1 AND cc."id" = $2 LIMIT 1`,
	...
	const allowedRoleIds = Array.isArray(channel.allowedRoleIds)
		? channel.allowedRoleIds.filter((roleId): roleId is string => typeof roleId === 'string')
		: [];

	const members = await db.query<Array<{ userId: string; baseRole: NookCommunityBaseRole }>>(
		`SELECT "userId", "baseRole" FROM "nook_community_member"
		 WHERE "communityId" = $1 AND "state" = 'active'`,
		[communityId],
	);
	if (channel.ownerId != null && !members.some(member => member.userId === channel.ownerId)) {
		members.push({ userId: channel.ownerId, baseRole: 'owner' });
	}
	...
	return members
		.filter(member => {
			...
			return (roleRowsByUserId.get(member.userId) ?? []).some(role =>
				...
				(Array.isArray(role.permissions) && role.permissions.some(permission => isNookCommunityPermission(permission) && permission === 'channels.manage')),
			);
		})
